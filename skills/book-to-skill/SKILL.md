---
name: book-to-skill
description: 拆书方法论 — Guide for creating effective skills. Also covers book-to-skill-converter (format-agnostic conversion from PDF/EPUB/DOCX/HTML/Markdown into structured Skill files). See references/converter.md.
---
...
### Claude Code 环境
- **PDF Skill**：`npx @anthropic-ai/claude-code-skills install pdf`
- **Swarm Skill**：多 Agent 并行处理不同章节（书籍超出上下文窗口时使用）

### Hermes Agent 环境
本技能从 Claude Code 生态部署到 Hermes Agent。在 Hermes 中使用时注意以下差异：

| 维度 | Claude Code | Hermes Agent |
|------|------------|-------------|
| 技能安装路径 | `~/.claude/skills/<name>/` | `~/.hermes/skills/<name>/` |
| 安装命令 | `npx skills add <repo>` | `hermes skills install <url>` 或 <br>手动克隆 + 复制 SKILL.md |
| 召唤方式 | `/skill-name` | 加载对应 skill 后自然对话触发 |
| Swarm 多 Agent | Claude 内置 agents | 使用 `swarm` skill（Hermes 内置） |
| 上下文窗口 | ~200k tokens | 取决于当前模型（DeepSeek V4 ≈ 128k） |

**关键适配点：**
- 输出的 SKILL.md 格式通用（Agent Skills 开放标准），Hermes 和 Claude Code 公用
- Swarm 在 Hermes 中通过 `/swarm` 技能实现，不是内置功能
- 书籍页数估算依旧有效，但 Hermes 上下文窗口按 128k 估算
