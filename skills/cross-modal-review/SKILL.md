...
- **Brain-page quality concerns
- **User-specified model review** — when the user explicitly asks for a specific model (GPT 5.5, Gemini, etc.) to review code. See [references/nuwa-adversarial-review.md](references/nuwa-adversarial-review.md) for the NUWA API technique.** — when brain writes need validation
...
## Dual-model independent review (cross-comparison)

For high-stakes code audits, run TWO independent reviews on the same code
(using different providers/models), then cross-compare findings. See
[references/nuwa-adversarial-review.md](references/nuwa-adversarial-review.md)
for the full workflow, output format templates, and priority rules.

**When to use:** mathematical/numerical code, security-sensitive areas,
user explicitly requests multiple models, or after fixing a critical bug
