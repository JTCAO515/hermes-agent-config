---
name: figma-use
description: "Use_figma Plugin API skill for Figma Design, FigJam, and Slides. **MANDATORY prerequisite** — load this skill BEFORE every `use_figma` tool call. Also covers `create_new_file`. NEVER call `use_figma` or `create_new_file` directly without loading this skill first. Skipping it causes common, hard-to-debug failures. Trigger whenever the user wants to perform a write action or a unique read action that requires JavaScript execution in the Figma file context — e.g. create/edit/delete nodes, set up variables or tokens, build components and variants, modify auto-layout or fills, bind variables to properties, inspect file structure programmatically, create a new file, or work in FigJam or Slides mode."
...
| [figjam-mode.md](references/figjam-mode.md) | Working in FigJam (`figma.com/board/...`) | FigJam-specific node types, upload_assets, get_figjam, colors, stickies, sections, shapes |
| [slides-mode.md](references/slides-mode.md) | Working in Slides (`figma.com/slides/...`) | Slides-specific APIs, slide lifecycle, deck-building workflow, speaker notes |
| [create-new-file.md](references/create-new-file.md) | Creating a new blank Figma file | create_new_file tool usage, planKey resolution, editor types |
| [swiftui.md](references/swiftui.md) | SwiftUI ↔ Figma translation | Design → code and code → design workflows, iOS HIG tokens, SF Symbols |
