...
The `/understand` tool produces a `knowledge-graph.json`. The following subcommands operate on that graph. Each has detailed instructions in the corresponding `references/` file:

| Subcommand | Trigger | Reference | Purpose |
|-----------|---------|-----------|---------|
| `chat` | User asks questions about the codebase | [references/understand-chat.md](references/understand-chat.md) | Q&A using the knowledge graph |
| `dashboard` | User wants to visualize the graph | [references/understand-dashboard.md](references/understand-dashboard.md) | Launch web UI dashboard |
| `diff` | User asks about PR/git diff impact | [references/understand-diff.md](references/understand-diff.md) | Analyze changed files vs graph |
| `domain` | User wants business domain extraction | [references/understand-domain.md](references/understand-domain.md) | Extract domains, flows, process steps |
| `explain` | User wants deep-dive on a component | [references/understand-explain.md](references/understand-explain.md) | In-depth file/function/module explanation |
| `knowledge` | User provides a Karpathy-pattern wiki | [references/understand-knowledge.md](references/understand-knowledge.md) | Wiki → interactive knowledge graph |
| `onboard` | New team member needs orientation | [references/understand-onboard.md](references/understand-onboard.md) | Generate onboarding guide from graph |

**Usage:** When a user triggers any subcommand, load the `understand` skill, then read the corresponding reference for detailed instructions.
