...
## PDF Rendering (brain-pdf)

Render any brain page to a publication-quality PDF via the gstack `make-pdf` binary. The brain page is ALWAYS the source of truth; the PDF is a rendering.

### Prerequisite

gstack `make-pdf` binary at `$HOME/.claude/skills/gstack/make-pdf/dist/pdf`.

### Basic workflow

1. **Resolve**: Confirm the brain page exists (`gbrain get <slug>`)
2. **Strip**: Remove YAML frontmatter (sed to skip `---` blocks)
3. **Render**: `CONTAINER=1 "$P" generate "$CLEAN" "$OUT"` (no `--cover`, no `--toc` by default)
4. **Deliver**: Hand the PDF via the agent's preferred channel
...
gbrain get "$SLUG" > /dev/null || { echo "Page not found" >&2; exit 1; }
RAW=$(mktemp /tmp/brain-page-XXXXXX.md)
gbrain get "$SLUG" --raw > "$RAW"
...
- `--watermark DRAFT` for in-progress work
- `--cover --toc` only if explicitly requested
- `--title "..." --author "..."` for custom metadata

### Font requirements

`fonts-liberation` (Helvetica substitute), `fonts-noto-cjk` (CJK characters). Body text 11pt minimum.

### Anti-Patterns

- Generating without confirming the brain page exists first
- Skipping frontmatter strip (renders as raw metadata text)
- Adding cover/TOC by default (off unless asked)
