...
> **⚠️ 中国大陆网络注意**：`openapi.moomoo.com` (43.174.250.223) 从部分海外服务器访问会被 Connection reset。如果下载 skills 或文档时遇到连接失败，将域名替换为 `openapi.futunn.com`。技能包下载示例：
> ```
...
| Linux | tar.gz → AppImage（优先） 或 deb/rpm 包 | curl 下载解压，AppImage 拷贝至 /opt 并创建 symlink；旧版 deb 用 dpkg 安装、rpm 用 rpm 安装 | `scripts/install_linux.md` |
...
- **⚠️ protobuf 版本强制降级**：`futu-api` 要求 `protobuf>=3.0,<4.0`，但 `pip install futu-api` **不会自动降级 protobuf**，可能安装 5.x/6.x。SDK 升级后必须**显式降级**，否则连接会**静默超时**（TCP 端口通但 `OpenQuoteContext` 永不返回）：

  ```bash
  $PIP install "protobuf>=3.0,<4.0"
  ```

  **症状识别**：`check_env.py` 通过（只做 socket 检查）、端口监听、TCP 可连 — 但任何 SDK 连接操作都超时 → 99% 是 protobuf 版本问题。

- 其他依赖 `protobuf >=5.0` 的包（如 `opentelemetry-proto`）会警告不兼容，但不影响 futu-api
- 如果用户环境中有其他依赖 `protobuf 4.x` 的包，提醒可能存在冲突，建议使用虚拟环境

## 常用依赖库安装
  ```

  **症状识别**：如果以下检查全部通过但仍连不上 OpenD：
  - `check_env.py` 通过（socket 检查 OK）
  - `ss -tlnp | grep 11111` 显示端口在监听
  - `nc -w 3 127.0.0.1 11111` TCP 连接成功
并提示 SDK 版本是否与 OpenD 版本匹配。
...
| 云服务器无图形界面 | 使用 Xvfb + VNC 方案部署，详见 `references/headless-deployment.md` |
