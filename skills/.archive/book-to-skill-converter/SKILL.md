...
此技能安装于 **Hermes Agent** (`~/.hermes/skills/book-to-skill-converter/`)。原 SKILL.md 面向 Copilot CLI / Claude Code / Amp 生态，以下是 Hermes 专属配置。

### Hermes 文档→技能转换工作流（手工萃取）

当用户发送文档（DOCX/PDF/EPUB/MD）并期望转换为 Hermes Skill 时，**不运行 extract.py 管线**（它输出的是 Copilot/Amp 格式）。而是执行以下 Hermes 专属流程：
...
book = epub.read_epub('path/to/file.epub')
...
**步骤 2：分析内容结构**
- 识别核心框架、原理、技术方法、反模式
- 判断内容类型：技术类（代码/表格）vs 文本类（叙事/方法论）
- 估算章节数和总 token 量

**步骤 3：创建 Hermes Skill**
1. 在 `~/.hermes/skills/<slug>/` 下创建 `SKILL.md`
2. 写入标准 Hermes frontmatter：
   ```yaml
   ---
   name: <kebab-case-slug>
   version: 1.0.0
   description: "<中文/英文描述，30字内>"
   triggers:
     - "<触发词1>"
     - "<触发词2>"
   ---
   ```
3. 正文萃取结构化知识：框架 → 代码模板 → 反模式 → 设计模式
4. 可选：添加 `references/` 目录存放引用资料、`templates/` 存放模板、`scripts/` 存放脚本

**步骤 4：验证**
```bash
ls ~/.hermes/skills/<slug>/SKILL.md
hermes skills list | grep <slug>
```

> ⚠️ **注意：** 当 `book-to-skill-converter` 与 `book-to-skill` (apple-ouyang 版) 共存时，务必使用 `book-to-skill-converter` 名称避免冲突。apple-ouyang 版提供的是预制决策技能，当前版提供的是转换工具。

### Hermes 中 extract.py 脚本路径

当需要运行 extract.py 时（例如处理大型技术 PDF），脚本位于：
```bash
~/.hermes/skills/book-to-skill-converter/scripts/extract.py
```
在 Hermes 中不需要 `PYTHON_BIN` 检测 — 固定使用 `python3`。

### 实战参考
见 `references/hermes-docx-to-skill-example.md` — 记录了 2026-06-16 将一篇 DOCX 中文文档转换为 Hermes Skill 的完整执行过程。

### Hermes 依赖安装
```bash
# 文字提取基础依赖
pip3 install python-docx ebooklib beautifulsoup4 lxml PyPDF2 pdfminer.six striprtf

# 技术类 PDF 需要 poppler-utils
