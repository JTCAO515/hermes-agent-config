...
## Related Sub-Skills (Absorbed)

This umbrella skill absorbed sibling skills from the skill lifecycle pipeline. Their content is preserved as reference files.

### Skill Creator (`references/skill-creator.md`)
SKILL.md frontmatter template and section requirements (Contract, Phases, Output Format, Anti-Patterns). Use for the scaffolding step in Phase 1-2 of skillify. Covers MECE checks against existing skills and manifest/resolver updates.

### Skillpack Check (`references/skillpack-check.md`)
Health check for the gbrain installation via `gbrain skillpack-check` (exit-code gated JSON report). Use in daily cron, after container restart, or proactively when jobs or brain stop working. Four exit states: healthy / action-needed / indeterminate.

### Skillpack Harvest (`references/skillpack-harvest.md`)
Editorial workflow for lifting a proven skill from a host repo (e.g. OpenClaw fork) back into gbrain's bundle. Five-phase pipeline: plan → dry-run → genericization (scrub real names, generalize triggers) → real harvest → verify. Always run the privacy linter.

### Skill Vetter (`references/skill-vetter.md`)
Security-first vetting protocol for evaluating skills before installation. Covers source check, code review red flags, permission scope analysis, and risk classification (LOW/MEDIUM/HIGH/EXTREME). Use before installing any external skill — complementary to skillify's creation pipeline.

### Find Skills (`references/find-skills.md`)
Discovery guide for finding and installing external skills via the `npx skills` ecosystem. Covers CLI commands (`find`, `add`, `check`, `update`), common skill categories, and what to do when no matching skill exists. Part of the end-to-end skill lifecycle (discover → vet → install → create).
