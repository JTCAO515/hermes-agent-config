...
### Sourcing from External Animation Libraries

When a user asks to add effects/animation components from an external library (React Bits, Magic UI, Aceternity, etc.) to an existing project:

**Curation workflow**: Browse the library's catalog, categorize candidates by where they'd fit in the target project (hero bg, cards, navigation, cursor effects, text effects), then rank by impact. Present a priority-ordered shortlist with usage suggestions — let the user decide which to implement before writing code.

**Clone-first rule**: NEVER modify the original production repo directly. Clone it to a new directory (or fork) first, then work on the copy. This preserves the original and lets the user compare before merging.

**Framework adaptation**: Most animation libraries target React. Check whether the target project uses the same framework:
- If yes → install via CLI or copy the component code directly
- If no (vanilla JS) → adapt the visual effect to standalone JS classes with CSS (see next section). The library's source code is a reference for the animation logic, not a direct import.

**Check the library's framework options**: Some libraries ship JS+CSS variants alongside React. Always check for these before adapting from React. Example: React Bits supports JS+CSS, TS+CSS, JS+Tailwind, TS+Tailwind per component.

> Full implementation patterns from a React Bits → vanilla JS adaptation → `references/visebits-patterns.md`

### Vanilla JS Animation Components (React Component Adaptation)

When adapting React-based animation/effect components (from libraries like React Bits, Magic UI, Aceternity) to vanilla JS projects:

**Architecture pattern**: Create a standalone `effects.js` + `effects.css` pair, independent of the main app code. Each effect is a self-contained ES6 class that manages its own DOM/CANVAS lifecycle.

**Component patterns**:

| Type | Technique | Key APIs |
|---|---|---|
| **Canvas background** (aurora, beams, particles) | `requestAnimationFrame` loop drawing to `<canvas>`; handle `devicePixelRatio` scaling; resize on window resize | `CanvasRenderingContext2D`, `requestAnimationFrame`, `devicePixelRatio`, `createRadialGradient` |
| **3D hover cards** (tilt) | CSS `perspective()` + `rotateX()`/`rotateY()` transform on mousemove; disable on touch devices via `window.matchMedia('(pointer: coarse)')` | `getBoundingClientRect`, CSS `transform: perspective() rotateX() rotateY()`, `pointer: coarse` media query |
| **Spotlight cards** | CSS `radial-gradient` positioned via custom properties `--mouse-x`/`--mouse-y`; pseudo-element `::before` with opacity transition | CSS custom properties, `radial-gradient`, `::before` pseudo-element |
| **Count-up (scroll-triggered)** | `IntersectionObserver` to trigger once; `requestAnimationFrame` with eased progress function (`1 - pow(1-t, 3)`) over a fixed duration; `font-variant-numeric: tabular-nums` | `IntersectionObserver`, `requestAnimationFrame`, cubic ease-out |
| **Click particle effects** (splash, sparkle) | Fixed `<canvas>` overlay with `pointer-events: none`; particles array with physics (position, velocity, gravity, alpha decay); `click`/`touchstart` listeners | Canvas overlay, particle physics (vx/vy/gravity/alpha), `devicePixelRatio` |

**DOM integration for dynamic content**: Use `MutationObserver` to watch for dynamically-added target elements and apply effects automatically. Use a `data-` attribute flag to prevent double-initialization.
...
**Framework independence**: Write all effects as standalone classes with no React/Vue dependency. CSS effects use only CSS custom properties and standard animations. Canvas effects use only the Canvas 2D API. This makes them portable to any framework.

> Full session patterns from a VisePanda React Bits adaptation → `references/visebits-patterns.md`
