...
## Mobile App Mockups (Phone-Sized Viewports)

When sketching mobile app screens (phone-form-factor), adapt the defaults:

### Viewport & sizing
- Set `max-width: 390px` (iPhone 14 size) or match the target device. Center the mockup with `margin: 0 auto`.
- Use a mobile-first layout — single column, full-width elements, no sidebar or multi-column assumptions.
- Include a **status bar** simulation (time, battery icons) at the top for realism.
- Add a **bottom navigation bar** if the app has tab navigation — this is a key visual differentiator between variants.

### Content density
- Mobile screens have less real estate — 3-4 content sections max per screen.
- Horizontal scrolling (`overflow-x: auto`) is native to mobile and OK for city cards / previews.
- Cards should be swipable, not grid-packed.

### HTML structure template for mobile mockups

```html
<body class="bg-[#0A0A0A] text-white min-h-screen">
  <!-- Status Bar -->
  <div class="flex justify-between items-center px-6 py-3 text-xs text-[#8B8B7A]">
    <span>9:41</span>
    <div class="flex gap-2"><span class="w-3.5 h-2 border rounded-sm"></span><span>🔋</span></div>
  </div>

  <!-- All content within max-w-[390px] centered -->
  <div class="max-w-[390px] mx-auto px-6">
    <!-- Hero / Header section -->
    <!-- Content sections (overflow-x-auto for scrollable rows) -->
    <!-- Essentials grid (3-col grid) -->
  </div>

  <!-- Bottom Navigation (fixed at bottom) -->
  <div class="fixed bottom-0 left-0 right-0 bg-[#1A1A1A] border-t px-4 py-2 max-w-[390px] mx-auto">
    <!-- Tab items: 4 items, flex justify-around -->
  </div>
  <div class="h-24"></div> <!-- spacer -->
</body>
```

### Mobile interactivity minimum
- Active/hover states on touch-friendly targets (min 44px height for buttons, 36px for nav)
- Horizontal scroll areas need `-webkit-overflow-scrolling: touch` and `scrollbar-hide` class
- Bottom nav FAB button (center tab) should visually pop — this is often the most important CTA on mobile

### Variant differentiation for mobile
Good axes for mobile app variants:
- **Theme:** dark vs light background
- **Accent personality:** warm gold vs cool blue vs neutral grey
- **Card style:** gradient overlays vs transparent borders vs solid shadows
- **Bottom nav treatment:** flat icons vs pill-style FAB vs elevated tab bar
- **Typography mood:** sans-serif utilitarian vs serif editorial vs condensed modern
...
### browser_vision fallback (when model doesn't support image analysis)

Some LLM providers/models do not support `image_url` in messages — `browser_vision` will return an error with a `screenshot_path` even though the screenshot was captured. Do NOT treat this as a failure of the sketch workflow:

1. The screenshot was captured — find it at `screenshot_path` in the error response
2. Share it with the user via `MEDIA:/path/to/screenshot.png`
3. Use `browser_navigate()` to check the page's accessibility tree instead:
   ```
   browser_navigate(url="file:///path/to/variant/index.html")
   ```
   The returned snapshot shows all rendered headings, buttons, and interactive elements — enough to confirm structure is correct.
4. If you need to verify visual details (colors overlap, spacing), use `browser_click()` on elements to confirm they respond, and check the CSS visually by inspecting the snapshot's element descriptions.

Don't skip presenting the variant just because vision analysis failed — the HTML mockup is still valid and valuable for user comparison.
