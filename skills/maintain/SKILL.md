...
The full troubleshooting guide: `docs/guides/minions-fix.md`.

### Citation audit and fix

Via `references/citation-fixer.md` — a dedicated workflow for auditing
and fixing citation formatting across brain pages. Extends the basic
citation audit above with:

- **Broken tweet/post references**: scan for references to X/Twitter
  posts that lack actual URLs, resolve them via X API integration
- **Batch mode**: sweep many pages with rate-limited resolution
- **Priority ordering**: fresh pages first, high-traffic pages next
- **State tracking**: `~/.gbrain/citation-fixer-state.json` for cron

Use when:
- The user asks to fix citations across the whole brain
- Tweet/post references lack x.com URLs
- A weekly cron sweep is needed for citation hygiene
