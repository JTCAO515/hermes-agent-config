...
### 截图工具
```bash
node assets/capture.js    [fullpage]
```
从 skill 根目录运行。依赖 skill 根目录下的 `node\_modules/` 中的 playwright。如报错：
```bash
npm install playwright && npx playwright install chromium
```
### Footer
- 左侧：logo + 李继刚（已硬编码在模板中）
- 右侧：内容来源（可选）——有明确来源时显示（如作者名、arxiv ID、网站名等），无来源时留空。使用 `{{SOURCE\_LINE}}` 变量：有来源时填 `来源文字`，否则空字符串。适用于 `-l`、`-i`、`-v`、`-c`、`-w` 模具（`-m` 多卡无 footer，不适用）。
### 交付
1. 报告文件路径
## 品味准则
